...
struct Code {
  ...
  int coding() {
    hash = 0;
    for (int i = 1; i <= 3; i++)
      for (int j = 1; j <= 3; j++) hash = hash * 10 + e[i][j];
    return hash;
  }
  void decoding() {
    for (int i = 3; i >= 1; i--)
      for (int j = 3; j >= 1; j--) e[i][j] = hash % 10, hash /= 10;
  }
  Code(int x, char k[][4], int b, char fc) {
    hash = x;
    memcpy(e, k, sizeof(char) * 16);
...
void print_pre(Code x, int m, int n) {
  ...
  if (h[t][k].f == 's')
    ...
    if (h[t][k].f == 'u')
      swap(x.e[m + 1][n], x.e[m][n]), print_pre(x, m + 1, n);
    if (h[t][k].f == 'd')
      swap(x.e[m - 1][n], x.e[m][n]), print_pre(x, m - 1, n);
    if (h[t][k].f == 'l')
      swap(x.e[m][n + 1], x.e[m][n]), print_pre(x, m, n + 1);
    if (h[t][k].f == 'r')
      swap(x.e[m][n - 1], x.e[m][n]), print_pre(x, m, n - 1);
  }
  putchar(h[t][k].f);
...
void print_post(Code x, int m, int n) {
  ...
  if (h[t][k].f == 's')
    ...
    if (h[t][k].f == 'u')
      swap(x.e[m + 1][n], x.e[m][n]), print_post(x, m + 1, n);
    if (h[t][k].f == 'd')
      swap(x.e[m - 1][n], x.e[m][n]), print_post(x, m - 1, n);
    if (h[t][k].f == 'l')
      swap(x.e[m][n + 1], x.e[m][n]), print_post(x, m, n + 1);
    if (h[t][k].f == 'r')
      swap(x.e[m][n - 1], x.e[m][n]), print_post(x, m, n - 1);
  }
}
