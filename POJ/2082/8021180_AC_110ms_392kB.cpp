...
bool solve(){
	while ( scanf ("%d", &n) == 1 && n != -1 ) {
		l.top = 0; int temp1 , temp2 ; scanf ("%d%d", &temp2, &temp1 ); l.s[++l.top].h = temp1 ; l.s[l.top].w = temp2 ;ans = temp1; n -- ; 
		while ( n -- ) {
			int key, keyw , w = 0; scanf ("%d%d", &keyw,&key ) ;  ans = max ( ans , key ) ; //printf("%d : ",key);
			while ( l.top && l.s[l.top].h >= key) update( w  );
			l.s[++l.top].h = key , l.s[l.top].w = w + keyw;
			//print();
		}
		int w = 0;
