...
bool bfs()
{
  ...
  while (!que.empty()) {
    t = que.top();
    que.pop();
   // printf("x=%d y=%d p=%d w=%d bx=%d by=%d\n", t.x, t.y, t.p, t.w, t.bx, t.by);
    if (t.x <= 0 || t.x > n || t.y <= 0 || t.y > m/* || vis[t.x][t.y][t.bx][t.by]*/) {
      continue;
    }
    ...
    for (int i = 0; i <= 3; ++i) {
      ...
      if (t.bx == xx && t.by == yy /*&&!vis[xx][yy][t.bx + dx[i]][t.by + dy[i]]*/) {
...
void input()
{
  ...
  //printf("%d %d %d %d %d %d", sx, sy, bx, by, tx, ty);
  return;
}
int kase=0;
void solve()
