...
bigint count(bigint p) {
  ...
  per (i,0,tot) {
    if (bin[i]==1) ret=ret+exi*fac[i]+num[i]+one,exi=exi+one;//,fac[i].print(),num[i].print();
  }
  return ret;
}
...
bigint findnxt(bigint p) {
  ...
  per (i,0,n+1) {
    ...
    //if (i==0) t.init(1),t=t+exi;
    if (t<=p) p=p-t,exi=exi+one,ret=ret+fac[i];
  }
  /*
  int tot=0;bigint t=ret+one; cnt=0;
  while (!(t.len==1&&t.s[0]==0)) bin[tot++]=t.s[0]&1,t=t/2,cnt+=bin[tot-1]==1;
  if (p.s[0]==cnt) ret=ret+one;
  //if (cnt>p.s[0]) ret=ret-one;
...
int main() {
  ...
  l=l/m;
  x.init(100);//count(x).print();//return 0;
  //judge(x);return 0;
  bigint r=l+x;
