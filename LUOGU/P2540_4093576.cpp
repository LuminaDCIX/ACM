...
int fc() {
  memset(c,0,sizeof(c));
  int k=0;
  rep (i,0,mx+1) if (cnt[i]>4) ++k;
  else c[cnt[i]]++;
    


  return f[c[4]][c[3]][c[2]][c[1]]+k;
}
...
void calc(int n4,int n3,int n2,int n1,int now,int lft) {
  ...
  if (n4>=2) n4-=2,calc(n4,n3,n2,n1,now+1,lft-8),n4+=2;
  if (n4&&n1>=2) n4-=1,n1-=2,calc(n4,n3,n2,n1,now+1,lft-6),n4+=1,n1+=2;
  if (n4&&n2) n4-=1,n2-=1,calc(n4,n3,n2,n1,now+1,lft-6),n4+=1,n2+=1;
  ...
  if (n4) n4-=1,n2+=2,calc(n4,n3,n2,n1,now,lft),n4+=1,n2-=2;
  if (n4) n4-=1,n3+=1,n1+=1,calc(n4,n3,n2,n1,now,lft),n4+=1,n3-=1,n1-=1;
  if (n3) n3-=1,calc(n4,n3,n2,n1,now+1,lft-3),n3+=1;
  if (n3) n3-=1,n2+=1,n1+=1,calc(n4,n3,n2,n1,now,lft),n2-=1,n1-=1,n3+=1;
  if (n2) n2-=1,calc(n4,n3,n2,n1,now+1,lft-2),n2+=1;
  //if (n2) n2-=1,n1+=2,calc(n4,n3,n2,n1,now,lft),n2+=1,n1-=2;
...
int main() {
  ...
  while (T--) {
    ...
    //    rep (i,0,mx+1) cerr<<cnt[i]<<" ";cerr<<endl;
    ans=2e9;
    dfs(0,n);
    printf("%d\n",ans);
  }
